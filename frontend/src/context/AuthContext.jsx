...
export function AuthProvider({ children }) {
    ...
    useEffect(() => {
        const unsubscribe = onAuthStateChanged(auth, async (user) => {
            if (user) {
                // Try to sync with backend, but don't block if it fails.
                // Uses authAPI (axios instance configured with VITE_API_URL),
                // NOT a raw fetch('/api/...') - a relative path would hit
                // Vercel's own domain instead of the Render backend.
                try {
                    const res = await authAPI.syncProfile(user.uid);
                    const role = res.data?.data?.role || 'citizen';
                    setUserRole(role);
                    setCurrentUser({ ...user.toJSON(), ...res.data?.data });
                } catch (err) {
                    // Backend unavailable — set user from Firebase data
    ...
    const register = async (userData) => {
        ...
        try {
            ...
            console.warn('Backend registration failed (non-critical):', backendErr.response?.data?.message || backendErr.message);
        }
