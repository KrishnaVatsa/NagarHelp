...
function SOSBroadcast() {
  ...
  const user = currentUser ? {
    _id: currentUser._id || currentUser.uid,
    name: currentUser.displayName || currentUser.name || currentUser.email?.split('@')[0] || 'User'
  } : null;
  ...
  useEffect(() => {
    ...
    if (socket) {
      ...
      socket.on('sos_resolved', ({ debrief }) => {
        ...
        const isBroadcaster = currentSos?.broadcaster?._id === user?._id;
      ...
      // AI guidance/summary now generates in the background after SOS
      // creation responds immediately (faster broadcast confirmation).
      // This event fires once that background generation completes.
      socket.on('sos_guidance_ready', ({ guidance: readyGuidance, emergencySummary: readySummary }) => {
        setGuidance(readyGuidance);
        setEmergencySummary(readySummary || '');
      });
    }
