...
import { adminAPI, sosAPI, authAPI, whatsappAPI } from '../services/api';
...
import {
  ...
  Menu, Bell, LogOut, LayoutDashboard, Shield, Clock, Flag, Map as MapIcon, Eye, ClipboardList
} from 'lucide-react';
...
function AdminDashboard() {
  ...
  const [civicFilter, setCivicFilter] = useState('');
  const [civicPage, setCivicPage] = useState(1);
  const navigate = useNavigate();
  const { currentUser, logout } = useAuth();
  const user = { name: currentUser?.displayName || currentUser?.name || currentUser?.email?.split('@')[0] || 'Admin' };
  ...
  useEffect(() => {
    if (activeTab === 'analytics') fetchLocality();
    if (activeTab === 'sos') fetchSOSList();
    if (activeTab === 'map') fetchActiveSOS();
    if (activeTab === 'civic') fetchCivicList();
  }, [activeTab, sosFilter, sosPage, civicFilter, civicPage]);
  ...
  const fetchCivicList = async () => {
    try {
      const params = { limit: 20, page: civicPage, sortBy: '-createdAt' };
      if (civicFilter) params.status = civicFilter;
      const res = await adminAPI.getAllCivicIssues(params);
      setCivicIssues(res.data.data.issues || []);
    } catch (error) {
      console.error('Failed to load civic issues');
    }
  };
  ...
  const handleCivicStatusUpdate = async (issueId, newStatus) => {
    if (!window.confirm(`Mark this issue as ${newStatus}?`)) return;
    try {
      // Ask for an optional note for the user
      const note = window.prompt("Optional note for the reporter (will be sent via WhatsApp):");
      
      // Update locally first for snappiness
      setCivicIssues(civicIssues.map(i => i._id === issueId ? { ...i, status: newStatus } : i));
      
      // Update in DB
      await adminAPI.updateCivicIssue(issueId, { status: newStatus });
      
      // Notify via WhatsApp
      await whatsappAPI.notifyStatus({ issueId, newStatus, note });
      
    } catch (error) {
      console.error('Failed to update civic status', error);
      alert('Failed to update status');
      fetchCivicList(); // Revert on failure
    }
  };
    ...
    <div className="min-h-screen bg-gray-50 flex">
      ...
      `}>
        ...
        <div className="flex-1 p-4 space-y-2">
          ...
          <TabButton id="civic" label="Civic Reports" icon={ClipboardList} />
            ...
            >
              ...
              {activeTab === 'civic' && (
                <div className="space-y-4">
                  <div className="flex items-center gap-3 flex-wrap">
                    <span className="text-sm font-medium text-gray-600">Filter:</span>
                    {['', 'Draft', 'Pending', 'In-Progress', 'Resolved'].map((f) => (
                      <button
                        key={f}
                        onClick={() => { setCivicFilter(f); setCivicPage(1); }}
                        className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${civicFilter === f
                          ? 'bg-gray-900 text-white'
                          : 'bg-white border border-gray-200 text-gray-600 hover:border-gray-300'
                          }`}
                      >
                  ...
                  <div className="card-premium overflow-hidden">
                      ...
                      <table className="w-full text-sm text-left">
                        <thead className="bg-gray-50 text-gray-500 uppercase text-xs">
                          <tr>
                            ...
                            <th className="px-4 py-3 font-medium">Status</th>
                            <th className="px-4 py-3 font-medium">AI Verify</th>
                            <th className="px-4 py-3 font-medium">Reporter</th>
                            <th className="px-4 py-3 font-medium text-right">Verify Action</th>
                          </tr>
                        </thead>
                        <tbody>
                          {civicIssues.map((issue) => (
                            <tr key={issue._id} className="border-b border-gray-50 hover:bg-gray-50/50">
                              <td className="px-4 py-3">
                                {issue.images?.length > 0 ? (
                                  <img src={issue.images[0]} alt="issue" className="w-12 h-12 object-cover rounded-lg" />
                                ) : (
                                  <div className="w-12 h-12 bg-gray-100 rounded-lg flex items-center justify-center text-xs text-gray-400">No Img</div>
                                )}
                              </td>
                              <td className="px-4 py-3">
                                <div className="font-medium text-gray-900 truncate max-w-[200px]">{issue.title}</div>
                                <div className="text-xs text-gray-500">{issue.category}</div>
                              </td>
                              <td className="px-4 py-3">
                                <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase ${
                                  issue.status === 'Draft' ? 'bg-gray-100 text-gray-600' :
                              ...
                              <td className="px-4 py-3">
                                {issue.aiAnalysis?.isReal ? (
                                  <div className="flex flex-col">
                                    <span className="text-green-600 font-bold text-xs flex items-center gap-1"><CheckCircle size={12}/> Verified</span>
                                    {issue.aiAnalysis.confidence > 0 && <span className="text-[10px] text-gray-400">{issue.aiAnalysis.confidence}% conf</span>}
                                  </div>
                                ) : issue.aiAnalysis?.isReal === false ? (
                                  <span className="text-red-600 font-bold text-xs flex items-center gap-1"><XCircle size={12}/> Fake</span>
                                ) : (
                                  <span className="text-gray-400 text-xs">Unverified</span>
                                )}
                              </td>
                              <td className="px-4 py-3">
                                <div className="font-medium text-gray-700 text-xs">{issue.reportedBy?.name || 'Unknown'}</div>
                                <div className="text-[10px] text-gray-500">{new Date(issue.createdAt).toLocaleDateString()}</div>
                              </td>
                              <td className="px-4 py-3 text-right">
                                <div className="flex items-center justify-end gap-2">
                                  {issue.status !== 'Resolved' && (
                                    <button
                                      onClick={() => handleCivicStatusUpdate(issue._id, 'Resolved')}
                                      className="text-[10px] bg-green-50 text-green-700 border border-green-200 px-2 py-1 rounded hover:bg-green-100 font-bold uppercase"
                                    >
                                      Resolve
                                    </button>
                                  )}
                                  {issue.status === 'Pending' && (
                                    <button
                                      onClick={() => handleCivicStatusUpdate(issue._id, 'In-Progress')}
                                      className="text-[10px] bg-blue-50 text-blue-700 border border-blue-200 px-2 py-1 rounded hover:bg-blue-100 font-bold uppercase"
                                    >
                                      Verify
                                    </button>
                                  )}
                          ...
                          {civicIssues.length === 0 && (
                            <tr><td colSpan={6} className="text-center py-12 text-gray-400">No civic issues found</td></tr>
                          )}
                        </tbody>
                      </table>
                    </div>
                    <div className="flex items-center justify-between p-4 bg-gray-50 border-t border-gray-100">
                      <button
                        onClick={() => setCivicPage(Math.max(1, civicPage - 1))}
                        disabled={civicPage <= 1}
                        className="text-xs px-3 py-1.5 bg-white border rounded-lg disabled:opacity-50"
                      >
                        Previous
                      </button>
                      <span className="text-xs text-gray-500">Page {civicPage}</span>
                      <button
                        onClick={() => setCivicPage(civicPage + 1)}
                        disabled={civicIssues.length < 20}
                        className="text-xs px-3 py-1.5 bg-white border rounded-lg disabled:opacity-50"
                      >
                        Next
                      </button>
