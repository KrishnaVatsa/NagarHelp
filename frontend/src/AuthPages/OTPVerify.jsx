...
const OTPVerify = () => {
    ...
    const sendOtp = async (type, contact) => {
        ...
        try {
            const res = await authAPI.sendOTP(reqType, contact);
            const data = res.data;
            toast.success(`${type === 'mobile' ? 'WhatsApp' : 'Email'} OTP Sent!`);
        } catch (error) {
    ...
    const verifyOtp = async (type, contact, otp) => {
        ...
        setLoading(true);
        try {
            // Backend 'verify-otp' reads contact directly; type distinction
            // (whatsapp vs email) is inferred backend-side from contact format.
            const res = await authAPI.verifyOTP(contact, otp, uid);
            const data = res.data;
            ...
            if (type === 'mobile') {
                try {
                    await authAPI.syncProfile(uid, { joinCommunity: true });
                    toast.success("You've been added to your City Alert Hub!");
                } catch (syncErr) {
                    console.error("Community Join Error:", syncErr);
                }
            ...
            console.error('[OTP ERROR]', error.response?.status, error.response?.data);
