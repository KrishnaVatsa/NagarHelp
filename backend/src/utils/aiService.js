...
Return only the debrief text.`;
  ...
  const raw = await callGemini(prompt);
  if (raw) return raw.trim();
  return `Your ${crisisType} emergency has been resolved after ${minutes} minutes with ${responderCount || 0} responders. Please follow up with professional services if needed. Don't forget to rate your responders to help the community. Thank you for using NagarHelp.`;
}
...
export async function askCrisisAssistant(crisisType, question, conversationHistory = []) {
  ...
  const prompt = `You are NagarHelp AI, an emergency crisis assistant for India. The user is currently in a ${crisisType} emergency situation.
...
export async function verifyCivicImageWithAI(imageBase64, mimeType = 'image/jpeg') {
  if (!process.env.GEMINI_API_KEY) {
    console.warn('GEMINI_API_KEY not set - skipping image verification');
    return { isReal: true, category: 'Other', summary: 'Skipped AI verification' };
  }
  ...
  const prompt = [
    {
      ...
      parts: [
        {
          text: `You are an AI validating civic issues in India. Look at the attached image.
Does it clearly show a real civic issue (like a pothole, garbage dump, waterlogging, broken streetlight, open drain)?

Return ONLY a valid JSON object in this exact format:
{
  "isReal": true or false,
  "category": "Pothole|Garbage|Safety|Waterlogging|Streetlight|Drainage|Other",
  "summary": "Short 1-sentence description of what you see"
}` },
        {
          inlineData: {
            data: imageBase64,
  ...
  try {
    ...
    console.error('AI image verification error:', error.message);
    // Fallback if AI fails: assume it's valid so we don't block the user
    return { isReal: true, category: 'Other', summary: 'AI verification failed, accepted by default' };
  }
}
