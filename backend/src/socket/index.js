...
// Emitted once background AI guidance/summary generation finishes after
// SOS creation responded immediately (see sos.controller.js createSOS).
export const emitSOSGuidanceReady = ({ sosId, guidance, emergencySummary }) => {
  if (!ioInstance) return;
  ioInstance.to(`sos:${sosId}`).emit('sos_guidance_ready', { sosId, guidance, emergencySummary });
};
...
export const initializeSocket = (server) => {
  ...
  io.use(async (socket, next) => {
    ...
    try {
      ...
      const user = await User.findOne({ $or: [{ firebaseUid: decoded.uid }, { email: decoded.email }] });
      if (!user) {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('broadcast_sos', async ({ sosId }) => {
      try {
        ...
        const broadcasterUser = await User.findById(sos.broadcaster._id).populate('guardians', '_id name phone');
        ...
        const sendWaAlert = (phone, msg) => {
          if (phone) sendWhatsappMessage(phone, msg).catch(err => console.error("WA Alert failed:", err.message));
        };
        ...
        if (guardianIds.length > 0) {
          ...
          guardianIds.forEach(guardianId => {
            ...
            // WHATSAPP ALERT
            const gUser = broadcasterUser.guardians.find(g => g._id.toString() === guardianId);
            if (gUser && gUser.phone) {
              const waMsg = `🚨 *GUARDIAN SOS ALERT* 🚨\n\nYour ward ${broadcasterUser.name} triggered a ${sos.crisisType} emergency!\nLocation: ${sos.address || 'Unknown'}\n\nCheck the NagarHelp app immediately to respond!`;
              sendWaAlert(gUser.phone, waMsg);
            }
          });
          ...
          setTimeout(async () => {
            responders.forEach(responder => {
              ...
              // WHATSAPP ALERT
              if (responder.phone) {
                const waMsg = `🚨 *SOS ALERT NEARBY* 🚨\n\nEmergency Type: ${sos.crisisType}\nDistance: ${responder.distance ? responder.distance.toFixed(2) + 'km' : 'Nearby'}\n\nPlease check the NagarHelp app immediately to respond!`;
                sendWaAlert(responder.phone, waMsg);
              }
            });
          }, 15000);
