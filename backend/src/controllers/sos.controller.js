...
export const createSOS = asyncHandler(async (req, res) => {
  ...
  // Respond immediately — this is an emergency broadcast, the user must not
  // wait on AI calls (which can take several seconds) before confirmation.
  res.status(201).json(
    new ApiResponse(201, { sos, nearbyResources }, 'SOS created successfully')
  );

  // Generate AI guidance/summary in the background, after responding.
  // Errors here must never crash the request — it has already been sent.
  (async () => {
    try {
      const [guidance, emergencySummary] = await Promise.all([
        generateCrisisGuidance(crisisType, address),
        generateEmergencySummary(crisisType, address, normalizedRadius)
      ]);
      sos.aiGuidance = guidance;
      sos.emergencySummary = emergencySummary;
      await sos.save();
      emitSOSGuidanceReady({ sosId: sos._id, guidance, emergencySummary });
    } catch (err) {
      console.error('Background SOS AI guidance generation failed:', err.message);
    }
  })();
});
