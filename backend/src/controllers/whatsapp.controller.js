...
import { verifyCivicImageWithAI } from '../utils/aiService.js';
import { notifyAdminsAboutNewIssue } from '../services/notificationService.js';
import exifr from 'exifr';
      ...
      if (isImage) {
        try {
          ...
          if (verification.isReal) {
            ...
            // Notify admins (email/WhatsApp) — WhatsApp-reported issues have
            // no linked User record, so pass a lightweight stand-in for the name.
            notifyAdminsAboutNewIssue(existingDraft, { name: `WhatsApp reporter (${from})` })
              .catch((err) => console.error('Admin notify error:', err.message));

            let confirmationMsg = `✅ *Issue Verified & Submitted!*\n\nOur AI confirmed your report (${verification.summary}).\n\n📋 *Issue ID:* ${existingDraft._id.toString().slice(-8).toUpperCase()}\nStatus: Under Review\n\nTrack at: ${process.env.FRONTEND_URL}/civic/feed`;
